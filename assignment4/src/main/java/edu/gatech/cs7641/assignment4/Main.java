...
public class Main {
	...
	public static void main(String[] args) {
		...
		gridWorldDomain.setProbSucceedTransitionDynamics(1);
		...
		switch (algorithm) {
			...
			default:
				runAlgorithm(analysis, problem, domain, hashingFactory, initialState, new PlannerFactory() {
					...
					@Override
					public Planner createPlanner(int episodeIndex, SADomain domain, HashableStateFactory hashingFactory, SimulatedEnvironment simulatedEnvironment) {
						QLearning agent = new QLearning(domain, 0.99, hashingFactory, 0, 0.1);
						agent.setLearningPolicy(new BoltzmannQPolicy (agent ,30));
//						agent.setLearningPolicy(new EpsilonGreedy(agent,0.8));
	...
	private static void runAlgorithm(Analysis analysis, Problem problem, SADomain domain, HashableStateFactory hashingFactory, State initialState, PlannerFactory plannerFactory, Algorithm algorithm) {
		...
		for (int episodeIndex = 1; episodeIndex <= problem.getNumberOfIterations(algorithm); episodeIndex++) {
			 ...
			 */
			int maxNumberOfSteps = problem.getWidth() * problem.getWidth()*5;
	...
	private static Problem createProblem1() {
		...
		hazardRewardsHashMap.put(HazardType.SMALL, -10.0);
		hazardRewardsHashMap.put(HazardType.MEDIUM, -20.0);
		hazardRewardsHashMap.put(HazardType.LARGE, -30.0);
	...
	private static Problem createProblem2() {
		...
		HashMap<HazardType, Double> hazardRewardsHashMap = new HashMap<HazardType, Double>();
		hazardRewardsHashMap.put(HazardType.SMALL, -5.0);
		hazardRewardsHashMap.put(HazardType.MEDIUM, -50.0);
		hazardRewardsHashMap.put(HazardType.LARGE, -100.0);

		return new Problem(map, numIterationsHashMap, -1, 1000, hazardRewardsHashMap);
	}
